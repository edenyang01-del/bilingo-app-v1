...
export default function App() {
    ...
    const silentCheckAndRepair = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data && Array.isArray(data.stations) && data.repairedCount > 0) {
            console.log(`[Silent Auto-Repair] Silently repaired ${data.repairedCount} radio stream URLs`);
            handleUpdateStations(data.stations);
          }
        }
      } catch (e) {
        // Silent background check - no UI popups or notifications, strictly non-blocking
      }
    };
  ...
  useEffect(() => {
    ...
    if (activeStation?.streamUrl) {
      ...
      fetch('/api/set-active-station', {
        ...
        .catch((err) => console.warn('Offline or background station sync bypassed:', err))
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-blue-600 selection:text-white pb-12 transition-colors duration-200 ${appBgClass}`}>
      ...
      {/* Top Mobile Status Bar Safe Area Spacer (Preserves Phone Native Dark Background Color #0b0f19 regardless of app light/paper theme) */}
      <div 
        aria-hidden="true" 
      ...
      <header 
        className={`backdrop-blur-md border-b sticky z-40 shadow-lg transition-colors duration-200 py-1.5 ${headerClass}`}
